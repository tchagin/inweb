                    ...
                    @if(count($items))
                        ...
                        @foreach($items as $item)
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex">
                                        <a href="{{ route('admin.categories.edit', ['category' => $item->id]) }}" class="btn btn-sm btn-info float-left mr-2">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <form action="{{ route('admin.categories.destroy', ['category' => $item->id])  }}" method="post" class="float-left">
                                            @csrf
                                            @method('DELETE')
